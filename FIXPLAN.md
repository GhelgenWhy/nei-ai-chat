...
**Ревизия 4 (финальная по данным инспектора с телефона):** плагин `nei-layout-inspector` (отдельный репо) дал точную геометрию: `ih=904`, пилюля `.mobile-navbar` на **807..859**, `env(safe-area-inset-bottom)=0` — вебвью продолжается за системные кнопки Android, и пилюля **не достаёт до нижней кромки панели** (859 < 904−2). Прежнее условие «touches bottom» отбрасывало её → inset=0, ввод под пилюлей и системными кнопками. Фикс: хром = полоса, у которой верх в нижней **половине**, а низ доходит до нижней **четверти** панели; зазор под самым низким хромом (45px) запоминается как «системная область» и используется, когда пилюля автоспряталась. Ожидаемый inset в главном окне ≈ 97px. В drawer'е плашки `workspace-drawer-tab-select/header` — статные, место занимают, не перекрывают (чинить нечего); `workspace-drawer-backdrop` (z=5) под drawer'ом — не затеняет контент. Регресс-тесты на реальной геометрии телефона. 89 тестов, сборка зелёные.

**Ревизия 3 (после проверки на телефоне):** глубокое сканирование поймало не только пилюлю навбара, но и **полнэкранный fixed-слой** (затемнение/подложка drawer'а на мобиле) — inset стал равен высоте панели: пропало окно чата, ввод уехал под шапку. Фикс: chrome — только **полоса, начинающаяся в нижней половине** контейнера (`rect.top ≥ top + height/2`), фильтр применён и в глубоком скане, и в быстром пути. Регресс-тесты: full-screen backdrop → 0. Плюс фикс переполнения `ModelCapabilityBar` на узких контейнерах (`overflow: hidden` + переносимый счётчик токенов).

**Ревизия 2 (по скриншотам с телефона):** первый chrome-inset не сработал на мобиле — пилюля внизу экрана оказалась новым **плавающим auto-hiding навбаром Obsidian** (форум: «Mobile UI: Option to restore former fixed bottom navigation bar»), а не `.status-bar`/`.mobile-toolbar` из первого списка селекторов. Решение теперь не зависит от имён классов: `scanBottomOverlaps()` обходит DOM и находит **любой** видимый `fixed/absolute`-элемент, перекрывающий нижнюю кромку панели (h≥16, w≥80, исключая своё поддерево и транзиентные `.notice`/`.menu`/`.modal-container`); известные классы (включая `.mobile-navbar`) остались быстрым путём. Пересчёты: mount, resize, visualViewport, focusin/focusout c отложенными замерами, debounced MutationObserver на `body`. Это не «кривой движок Xiaomi» и не скейлинг — элемент просто не измерялся. Точка отката: коммит `d6cd3b6`.

**Ревизия B26 (после проверки на реальном Obsidian):** пункт B26 аудита оказался ошибочным — статус-бар Obsidian **плавает поверх** листьев воркспейса и на ПК, и на мобильных (подтверждено форумом Obsidian), а `.mobile-toolbar` над клавиатурой перекрывает низ панели. Статический паддинг заменён на рантайм-измерение (`src/utils/obsidianChrome.ts`). Паттерн оформлен как навык `obsidian-plugin-ui` (`~/.agents/skills/`, вместе с копией хелпера).
